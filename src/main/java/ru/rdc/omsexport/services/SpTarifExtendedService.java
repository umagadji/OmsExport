...
public class SpTarifExtendedService {
    ...
    public void saveSpTarifExtendedList() {
        ...
        "select " +
                "new SpTarifExtended (spt.name_issl, spt.ksg, spt.price, spt.type, spt.idpr, spt.kol_usl, spt.t_type, sptadd.ex_7_2, pf.prname, case when ukrmulti.ksg is null then 0 else 1 end, case when ukrmulti.ksg is null then 0 else ukrmulti.max_krat end, uslidsp.idsp, case when spt.t_type in (29,39,71,72,73,74,75,76,77,78,79) or spt.ksg in('65000','65001') then false else true end)" +
                "from SpTarif spt " +
                "INNER JOIN SpTarifAdd sptadd ON spt.type=1 AND (spt.kol_usl=1 OR spt.kol_usl=1.92) AND spt.ksg=sptadd.ksg " +
                "INNER JOIN Profot pf ON spt.idpr = pf.idpr " +
